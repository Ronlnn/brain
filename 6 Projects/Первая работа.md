...
rows.sort((a, b) => {
    ...
    const priorityA = priorityOrder[clean(a[0])] ?? 0;
    const priorityB = priorityOrder[clean(b[1])] ?? 0;
    const priorityC = priorityOrder[clean(b[2])] ?? 0;
    return priorityB - priorityA - priorityC ; // Сортируем по убыванию приоритета
});
